...
import {
  LoadContextService,
  AppendContextTurnParams,
} from '../chatbot/context/load-context.service';
...
export class LineDeliveryService implements OnModuleInit, OnModuleDestroy {
    ...
    try {
      ...
      for (const row of rows) {
        try {
          ...
          this.logger.error(`Delivery ${row.id}: ${String(error)}`);
        }
      }
    } catch (error) {
      this.logger.error(`Delivery recovery: ${String(error)}`);
    } finally {
      this.busy = false;
    }
  }
  ...
  async deliver(id: string): Promise<void> {
    let row = await this.prisma.lineDelivery.findUniqueOrThrow({
      where: { id },
    });
    ...
    if (row.status === 'SENDING' && row.method === 'REPLY') {
      await this.finishAttempt(id, owner, {
        status: 'UNKNOWN',
        lastError: 'Worker stopped during reply; acceptance unknown',
      });
      return;
    }
    ...
    try {
      // Drop an automatic answer queued before an admin took over.
      if (!row.adminMemberId && (await this.sessions.isMuted(row.lineUserId))) {
        await this.finishAttempt(id, owner, {
          status: 'FAILED',
          lastError: 'Automatic reply suppressed during admin mute',
        });
        return;
      }
      if (
        ...
        try {
          ...
          if (!(error instanceof LineDeliveryError && error.replyTokenInvalid))
            throw error;
      ...
      if (
        row.firstPushAt &&
        Date.now() - row.firstPushAt.getTime() >= 23 * 3600_000
      ) {
        await this.finishAttempt(id, owner, {
          status: 'UNKNOWN',
          lastError: 'Push retry key is near expiry; manual review required',
        });
        return;
      }
      ...
      // Applied on actual attempts, including recovery, never on replay of an
      // already accepted delivery. A Redis failure prevents the admin send.
      if (row.adminMemberId) await this.sessions.mute(row.lineUserId);
      else if (await this.sessions.isMuted(row.lineUserId)) {
        await this.finishAttempt(id, owner, {
          status: 'FAILED',
          lastError: 'Automatic reply suppressed during admin mute',
        });
      ...
      await this.finishAttempt(id, owner, {
        status: retry
          ? 'PENDING'
          : error instanceof LineDeliveryError && error.outcome === 'REJECTED'
            ? 'FAILED'
            : 'UNKNOWN',
        method,
        nextAttemptAt: new Date(
          Date.now() + Math.min(60_000, 2_000 * 2 ** (row.attempts - 1)),
        ),
  ...
  private async accept(row: LineDelivery, owner: string) {
    ...
    if (updated.count) {
      ...
      try {
        await this.finalize(
          await this.prisma.lineDelivery.findUniqueOrThrow({
            where: { id: row.id },
          }),
        );
      } catch (error) {
        this.logger.error(
          `Accepted delivery ${row.id}: history recovery needed: ${String(error)}`,
        );
      }
    }
  }
  ...
  private finishAttempt(
    id: string,
    owner: string,
    data: Prisma.LineDeliveryUpdateManyMutationInput,
  ) {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.lineChatHistory.upsert({
        ...
        create: {
          ...
          lineMemberId: row.lineMemberId,
          sender: row.adminMemberId ? 'ADMIN' : 'SYSTEM',
          sentByAdminId: row.adminMemberId,
          messageType: 'TEXT',
          text: row.text,
          sentStatus: 'sent',
      ...
      await tx.lineConversation.updateMany({
        where: {
          ...
          lastMessageAt: { lte: row.acceptedAt ?? new Date() },
        },
        data: {
          lastMessage: row.text,
          lastMessageType: 'TEXT',
          lastMessageAt: row.acceptedAt ?? new Date(),
        },
      });
    });
    if (row.context) {
      const turn = row.context as unknown as AppendContextTurnParams;
      if (turn.response.contextPolicy === 'INCLUDE')
        await this.context.appendTurn(turn);
      // CLEAR is done when the response is persisted, not on delayed delivery repair.
    }
    await this.prisma.lineDelivery.update({
      where: { id: row.id },
